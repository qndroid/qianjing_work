package com.qianjing.finance.service.update;
...
public class UpdateService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		if (intent == null
				|| (intent != null && !intent.hasExtra("lastVersion"))) {
		...
		notifySchedule(getString(R.string.update_download_start),
	...
	private void startDownload() {
		UpdateManager.getInstance().startDownload(apkUrl, filePath,
				new UpdateDownloadListener() {
					...
					public void onPaused(int progress, int completeSize,
						...
						notifySchedule(
								...
								getString(R.string.update_download_failed_msg),
								0);
						deleteApkFile();
						stopSelf();// 停掉服务自身
					}
					...
					public void onFinished(int completeSize, String downloadUrl) {
						PrefManager.getInstance().putBoolean(
								PrefManager.HAVE_SHOW_REWARD, false);
						notifySchedule(
	 ...
	 * API21以上使用此方法
	 */
	private void notifyThatUnder21(String tickerMsg, String message,
			int progress) {
		...
		if (progress > 0 && progress < 100) {
			...
			contentView
					.setTextViewText(R.id.name, getString(R.string.app_name));
		...
		notification.setContentIntent(progress >= 100
				? getContentIntent()
	 ...
	 * API21使用此方法
	 */
	private void notifyThatExceedLv21(String tickerMsg, String message,
			int progress) {
		NotificationCompat.Builder notification = new NotificationCompat.Builder(
				this);
