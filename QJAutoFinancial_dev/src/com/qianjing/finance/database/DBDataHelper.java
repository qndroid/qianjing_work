...
public final class DBDataHelper {
     ...
     * @return
     */
    public ArrayList<BaseModel> select(String tableName, String showColumns,
            String selection, String selectionArgs, String orderBy, Class<?> cls) {
     ...
     * @param selectionArgs 与selection相对应，里面的字符串将替换selection里的"?", 构成完整的匹配条件，例如{"6","jack"}
     * @param orderby 排序参数
    ...
    private void changeToList(Cursor cursor, List<BaseModel> modules,
            Class<?> moduleClass) {
        // 取出所有的列名
     ...
     * 
     * @param table 要操作的表
     * @param module 数据模型，id设为自增，若没指定，则自动生成；若指定则插入指定的id, 但是在数据表已存在该id的情况下会导致插入失败，建议不指定id
    ...
    public boolean insert(final String table,
            final ArrayList<FundSearchModel> modules) {
        synchronized (dbHelper) {
    ...
    private ArrayList<ContentValues> moduleToContentValues(
            ArrayList<FundSearchModel> modules) {
     ...
     * 
     * @param table 要操作的表
     * @param whereClause 匹配条件，例如"id>?and name <>?",符合条件的记录将被删除， 不需要可以设为null，这样整张表的内容都会被删除
     * @param whereArgs 与selection相对应，里面的字符串将替换selection里的"?", 构成完整的匹配条件，例如{"6","jack"}
     * @return 数据表受影响的行数
     */
     ...
     */
    public long update(final String table, final String whereClause,
            final String[] whereArgs, final BaseModel module) {
        synchronized (dbHelper) {
            ContentValues values = moduleToContentValues(module);
