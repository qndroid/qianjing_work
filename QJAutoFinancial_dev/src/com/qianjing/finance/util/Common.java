...
public class Common {
    ...
    public static AssembleDetail parseAssembleDetail(String strJson) throws JSONException {
        ...
        AssembleDetail detail = new AssembleDetail(new AssembleBase(), new AssembleConfig(),
                new AssembleAssets(), new AssembleReminder(),new AssembleFixed());
    ...
    public static AssembleNewItemLayout createFundItem(final Activity activity, final Fund fund) {
        ...
        fundItem.initData(
                fund.color,
                fund.name,
                activity.getString(R.string.left_brackets) + fund.code
                        + activity.getString(R.string.right_brackets),
                StringHelper.formatString(String.valueOf(fund.rate * 100),
     ...
     * @return
     */
    public static String calculateMoney(String sum, double radio) {
        double sumDouble = 0;
        if (StringHelper.isNotBlank(sum)) {
            sumDouble = Double.parseDouble(sum);
        }
        BigDecimal value = new BigDecimal(sumDouble).multiply(new BigDecimal(radio));
        String money = StringHelper.formatString(String.valueOf(value),
