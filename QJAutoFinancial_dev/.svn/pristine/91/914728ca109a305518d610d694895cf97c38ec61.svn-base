...
public final class DBHelper extends SQLiteOpenHelper {
	...
	private DBHelper() {
		super(QApplication.getInsten(), DATABASE_NAME, null, 1);
	}
	...
	private void createAllTables(SQLiteDatabase db) {
		 ...
		 * 创建基金列表
		 */
		String[] cloumns = new String[]{FUND_CODE + TEXT_TYPE,
				FUND_ABBREV + TEXT_TYPE, FUND_SPELL + TEXT_TYPE,
	...
	public synchronized boolean insert(final String table,
			final String nullColumnHack, final ArrayList<ContentValues> values) {
		boolean result = true;
		SQLiteDatabase database = null;
		try {
			database = getWritableDatabase();
