...
/**
 * @Description:理财增值申购结果页面
 * @date 2015-4-28
 */
public class QuickBuyResultActivity extends BaseActivity {
	...
	private void initView() {
		...
		rightText.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				QuickBuyResultActivity.this.setResult(CustomConstants.SHUDDOWN_ACTIVITY);
				finish();
			}
		});
		...
		opdateTimeView.setText(DateFormatHelper.formatDate(opDateTime.concat("000"), DateFormat.DATE_9));
		profitTimeView.setText(DateFormatHelper.formatDate(profitTimeShow.concat("000"), DateFormat.DATE_1));
		profitWeekView.setText(DateFormatHelper.formatDate(profitTimeShow.concat("000"), DateFormat.DATE_6));
		profitMsgView.setCompoundDrawables(null, null, null, null);
		profitArriveView.setText(DateFormatHelper.formatDate(profitArriveShow.concat("000"), DateFormat.DATE_1));
		profitArriveWeekView.setText(DateFormatHelper.formatDate(profitArriveShow.concat("000"), DateFormat.DATE_6));
		profitArriveMsgView.setCompoundDrawables(null, null, null, null);

		bankNameView.setText(getString(R.string.dao_zhang_ying_hang_ka) + bank + getString(R.string.left_brackets) + getString(R.string.wei_hao)
		...
		takeMoneyValueView.setText("¥" + StringHelper.formatDecimal(Double.parseDouble(money)));
		feeMoneyValueView.setText("¥" + StringHelper.formatDecimal(Double.parseDouble(fee)));
		...
		profitMsgViewIcon.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				showHintDialog(getString(R.string.share_explain_title), getString(R.string.share_explain));
			}
		});
		profitArriveMsgViewIcon.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				showHintDialog(getString(R.string.profit_explain_title), getString(R.string.profit_explain));
			}
		});
		...
		if (infoList.size() != 0) {
			for (int i = 0; i < infoList.size(); i++) {
				setDetailsContent(infoList.get(i).abbrev, Float.parseFloat(infoList.get(i).fdsum), getString(stateStr[infoList.get(i).fdstate]),
	...
	private void requestMendCheck() {
		...
		upload.put("sid", sid);
		RequestManager.request(this, UrlConst.MEND_CHECK, upload, MendCheck.class, new XCallBack() {
			@Override
				...
				if (model.stateCode == ErrorConst.EC_OK) {
					...
					mendLayout.setOnClickListener(new View.OnClickListener() {
						...
						public void onClick(View v) {
							...
							intent.setClass(QuickBuyResultActivity.this, AssembleMendDetailActivity.class);
			...
			public void handleResponse(ResponseLotteryStatus response) {
				if (response != null) {
					...
					if (response.lottery != null && response.lottery.status) {
						...
						spManager.putString(PrefManager.ACTIVITY_LOTTERY_MESSAGE, response.lottery.strMessage);
	...
	private void setDetailsContent(String abbrev, float item2, String item3, int state, String reason) {
	...
	public void onBackPressed() {
		...
		setResult(ViewUtil.ASSEMBLE_BUY_RESULT_CODE, mIntent);
		finish();
	}
